...
def test_ga4_included_in_get_all_staging_customers(spark, pipeline_tables):
    ...
    ga4_customers = all_customers.filter(col("source") == "ga4_sessions") \
                                 .orderBy("email").collect()
...
def test_ga4_anonymous_users_excluded(spark, pipeline_tables):
    ...
    now = datetime.now()
    insert_rows(spark, STG_GA4_SESSIONS, [
        {"session_id": "s1", "client_id": "c1", "user_id": "user@example.com", "_staged_at": now},
        {"session_id": "s2", "client_id": "c2", "user_id": None, "_staged_at": now},
        {"session_id": "s3", "client_id": "c3", "user_id": "", "_staged_at": now},
        {"session_id": "s4", "client_id": "c4", "user_id": "another@example.com", "_staged_at": now},
    ])
